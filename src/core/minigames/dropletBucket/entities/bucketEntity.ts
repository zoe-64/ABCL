...
export class BucketEntity extends GameEntity {
  ...
  constructor(game: DropletCatchGame, x: number, y: number, width: number, height: number) {
  ...
  catches(droplet: DropletEntity): boolean {
    ...
    return dropletRight > bucketLeft && dropletLeft < bucketRight && dropletBottom > bucketTop && dropletTop < bucketBottom;
  }

  setTargetX(x: number): void {
    const halfWidth = this.width / 2;
    this.targetX = Math.max(halfWidth, Math.min(this.game.width - halfWidth, x));
  }
